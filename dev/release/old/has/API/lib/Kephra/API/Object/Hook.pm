...
sub add_before    { _add(@_, BEFORE)} # obj method HID code --> 0|call
sub add_after     { _add(@_, AFTER) } # obj method HID code --> 0|call
sub _add {
    ...
    $self->{HOOK()}{$method}{$slot}{$HID} = is_call($action) 
        ? $action 
        : Kephra::API::Call->new($action);
}

################################################################################

sub list_before   {_list(@_, BEFORE)}  # obj method          --> [HID]
sub list_after    {_list(@_, AFTER)}  # obj method          --> [HID]
sub _list {
...
sub get_before    {_get(@_, BEFORE)}  # obj method HID      --> 0|call
sub get_after     {_get(@_, AFTER)}  # obj method HID      --> 0|call
sub _get {
...
sub remove_before { _remove(@_, BEFORE)} # obj method HID      --> 0|call
sub remove_after  { _remove(@_, AFTER) } # obj method HID      --> 0|call
