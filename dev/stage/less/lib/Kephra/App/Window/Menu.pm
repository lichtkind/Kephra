...
sub mount {
    ...
    Wx::Event::EVT_MENU( $win, 12110, sub { $win->{'editor'}->Undo                 });
    Wx::Event::EVT_MENU( $win, 12120, sub { $win->{'editor'}->Redo                 });
    Wx::Event::EVT_MENU( $win, 12130, sub { $win->{'editor'}->fast_undo            });
    Wx::Event::EVT_MENU( $win, 12140, sub { $win->{'editor'}->fast_redo            });
    Wx::Event::EVT_MENU( $win, 12150, sub { $win->{'editor'}->total_undo           });
    Wx::Event::EVT_MENU( $win, 12160, sub { $win->{'editor'}->total_redo           });
    Wx::Event::EVT_MENU( $win, 12200, sub { $win->{'editor'}->cut                  });
    Wx::Event::EVT_MENU( $win, 12210, sub { $win->{'editor'}->copy                 });
    Wx::Event::EVT_MENU( $win, 12220, sub { $win->{'editor'}->Paste                });
    Wx::Event::EVT_MENU( $win, 12230, sub { $win->{'editor'}->replace              });
    Wx::Event::EVT_MENU( $win, 12240, sub { $win->{'editor'}->Clear                });
    Wx::Event::EVT_MENU( $win, 12300, sub { $win->{'editor'}->expand_selecton      });
    Wx::Event::EVT_MENU( $win, 12310, sub { $win->{'editor'}->shrink_selecton      });
    Wx::Event::EVT_MENU( $win, 12400, sub { $win->{'editor'}->duplicate            });
    Wx::Event::EVT_MENU( $win, 13100, sub { $win->{'editor'}->move_left            });
    Wx::Event::EVT_MENU( $win, 13110, sub { $win->{'editor'}->move_right           });
    Wx::Event::EVT_MENU( $win, 13120, sub { $win->{'editor'}->move_up              });
    Wx::Event::EVT_MENU( $win, 13130, sub { $win->{'editor'}->move_down            });
    Wx::Event::EVT_MENU( $win, 13300, sub { $win->{'editor'}->toggle_block_comment });
    Wx::Event::EVT_MENU( $win, 13310, sub { $win->{'editor'}->toggle_comment       });
    ...
    Wx::Event::EVT_MENU( $win, 14310, sub { $win->{'editor'}->marker_toggle        });
    Wx::Event::EVT_MENU( $win, 14320, sub { $win->{'editor'}->marker_prev          });
    Wx::Event::EVT_MENU( $win, 14330, sub { $win->{'editor'}->marker_next          });
    Wx::Event::EVT_MENU( $win, 14250, sub { $win->{'replacebar'}->replace_all      });
    Wx::Event::EVT_MENU( $win, 14310, sub { $win->{'editor'}->toggle_marker        });
    Wx::Event::EVT_MENU( $win, 14340, sub { $win->{'editor'}->delete_all_marker    });
    Wx::Event::EVT_MENU( $win, 14320, sub { $win->{'editor'}->goto_prev_marker     });
    Wx::Event::EVT_MENU( $win, 14330, sub { $win->{'editor'}->goto_next_marker     });
    Wx::Event::EVT_MENU( $win, 14400, sub { $win->{'editor'}->goto_last_edit       });
    Wx::Event::EVT_MENU( $win, 15100, sub { $win->{'editor'}->toggle_tab_usage     });
    Wx::Event::EVT_MENU( $win, 15410, sub { $win->{'editor'}->set_EOL_lf   } );
    Wx::Event::EVT_MENU( $win, 15420, sub { $win->{'editor'}->set_EOL_cr   } );
    Wx::Event::EVT_MENU( $win, 15430, sub { $win->{'editor'}->set_EOL_crlf } );
    #Wx::Event::EVT_MENU( $win, 15701 + $_, eval 'sub { Kephra::App::Editor::SyntaxMode::set($win->{editor},"'.$modes[$_].'")}') for 0 .. $#modes;
    Wx::Event::EVT_MENU( $win, 16110, sub { $win->{'editor'}->toggle_view_whitespace  });
    Wx::Event::EVT_MENU( $win, 16120, sub { $win->{'editor'}->toggle_view_eol          });
    Wx::Event::EVT_MENU( $win, 16130, sub { $win->{'editor'}->toggle_view_indent_guide   });
    Wx::Event::EVT_MENU( $win, 16140, sub { $win->{'editor'}->toggle_view_right_margin   });
    Wx::Event::EVT_MENU( $win, 16210, sub { $win->{'editor'}->toggle_view_line_nr_margin });
    Wx::Event::EVT_MENU( $win, 16220, sub { $win->{'editor'}->toggle_view_marker_margin  });
    Wx::Event::EVT_MENU( $win, 16310, sub { $win->{'editor'}->zoom_in                    });
    Wx::Event::EVT_MENU( $win, 16320, sub { $win->{'editor'}->zoom_out                   });
