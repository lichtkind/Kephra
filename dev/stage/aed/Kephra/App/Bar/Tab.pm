...
use Wx::AUI;
use Kephra::API  qw(app_window is_panel);
...
sub new {
	...
	$self->{'visual_2_internal_pos'} = []; # visual order of internal pos : vis -> int
	$self->{'internal_2_visual_pos'} = []; # internal order of visual pos : int -> vis
...
sub add_page {
	...
	for   (@{$self->{'visual_2_internal_pos'}}){ $_++ if $_ >= $position }
	splice @{$self->{'visual_2_internal_pos'}},  $position, 0, $position;
...
sub get_page_by_vis_nr {
	...
	return $self->GetPage( $self->{'visual_2_internal_pos'}[ $nr ] );
}
...
sub raised_vis_nr {
	my ($self) = @_;
	$self->{'visual_2_internal_pos'}[ $self->GetSelection ];
}
...
sub move_page_position_visually {          # for dnd only
	...
	my $position = splice @{$self->{'visual_2_internal_pos'}}, $from, 1;
	splice @{$self->{'visual_2_internal_pos'}}, $to, 0, $position;
	$self->refresh_internal_page_order();
}
...
sub move_page_visually  {               # for movements by keyboard
	...
	my $pos = $self->{'visual_2_internal_pos'}[ $from ];
	my $page = $self->GetPage( $pos );
	my $label = $self->GetPageText( $pos );
	my $visual = $self->{'visual_2_internal_pos'};
	...
	$self->{'visual_2_internal_pos'} = $visual;
	$self->refresh_internal_page_order();
	$self->SetSelection( $self->{'visual_2_internal_pos'}[$to] );
	$self->mount_events();
}
...
sub active_visual_pos     { $_[0]->{'internal_2_visual_pos'}[ $_[0]->GetSelection ] }
...
sub next_page_pos_rot_left{
	my ($self, $pos) = @_; # take in position of internal order
	$pos = $self->{'internal_2_visual_pos'}[ $pos ];
	$self->{'visual_2_internal_pos'}[$pos == 0 ? $self->rightmost_page_pos : $pos-1];
}
sub next_page_pos_rot_right{
	my ($self, $pos) = @_; # take in position of internal order
	$pos = $self->{'internal_2_visual_pos'}[ $pos ]; # translate to visual order
	$self->{'visual_2_internal_pos'}[$pos == $self->rightmost_page_pos ? 0 : $pos+1]; # go vis. right and translate back
}


sub raised_title { $_[0]->GetPageText( $_[0]->GetSelection ) }

sub set_page_title {
	my ($self, $label, $page) = @_;
	$page = $self->GetSelection unless defined $page;
	return say("need a Kephra::App::Panel or valid position number") 
...
sub refresh_internal_page_order {       # sync visual_page_order index with internal_page_order after each change
	...
	$self->{'internal_2_visual_pos'} = $internal;
}
...
sub remove_page {
	...
	my $visual_position = $self->{'internal_2_visual_pos'}[$internal_position];
	my $visual = $self->{'internal_2_visual_pos'};
