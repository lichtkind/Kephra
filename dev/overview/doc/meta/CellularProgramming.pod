...
=head2 Preamble
    ...
    This document describes a set of workflows using a peculiar directory structure.
    It evolved from the development of the Kephra editor.
    ...
    The word cell refers to an semi independent sub-project, which lives in 
    a directory path that reflects its role within the overall project.
    

=head2 Goals

In general CP aims at the golden mean between planning, iteration and spontaneity.
It is created to optimize (in that order):

=over 4
=item code quality
=over 4
=item understandable (clear and consistent naming - simple and fitting abstractions)
=item changeable (well organized files plus same as in "understandable")
=item high test coverage and compliance
=item well documented on every level of abstraction
=back

=item project organisation
=over 4
=item plan with detailed but changeable roadmap
=item minimize risks
=item enable specialized / parallel development with boundaries of ownership
=item tracking progress
=item precise estimate of effort (work hours) to achieve mile stone
=back

=item development speed
=over 4
=item selecting most productive task at hand
=item declutter and set right aims for each activity
=item max long term benefit of each activity
...
=head3 Basic Steps
    ...
    1) (Un-)structured attempt to define the goal on user level:
       collect ideas, features, sketch GUI, user strories, possible components.
       Only as precise to be a helpful guide but update (U).

    2) Systematize and precise goal by writing high level documentation.
       Use such docs early for sharing with coprogrammer and audience/boss. (U)

    3) Create dummy/empty classes, with dummy methods to get a component/class 
       structure that actually runs/compiles. Add method signatures when possible.
       Insert comments to mark unclear spots or to clearify what the code suppose
       to do at this position.
       This process helps to get feel for the code structure and what belongs where.
       It also assist greatly in finding fitting class and method names.
       Useless UML diagrams, flawed abstractions and misleading names are prevented
       before being able to do real damage. (U)

    4) Envision stages in which the program can be released in equal sized steps 
    ...
    6) Write own libraries of the project in packages as independent projects,
       to match requirements of the current stage. Write first docs, than code
       tests last.

    7) Write one prototype for each set of user features, to experiment with
       alternatives. Update libraries, stage plan and overview when necessary.

    8) Combine code from feature prototypes and release stage (major release).
       Move code of current stage to projects lib and bin dir.
       Keep stage directory for further bug fixes. 

    9) Proceed with next stage until project is completed.


     *U = Do update when goal/plan changes
