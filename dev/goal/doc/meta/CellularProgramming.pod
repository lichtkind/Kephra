...
In the stage cell is hardly any coding done, only rudimental smoke tests,
...
The ladder has numerous advantages over UML and other abstract specs:
...
because you need a valve for creativity. The only requirements are:
...
so forth. CP add just /dev to it, where all the action will take place.
The above mentioned directories will only ontain the current release.
...
Module and feature cells will be located like (/dev/module/cell_name/stage_name).
Functional protos like: /dev/function/topic/cell_name):

Every root directory of a cell and cell type will contain a index.txt
which gives an overview to its content.
