...
easier to reflected upon, plan and complete them - and a completed task
just feels good. The freed up attention can alse be used for creativity
...
cells decouple parts of the software development that can be decouped
...
There are 6 kinds (types) of cells. Dependending on the project size and
structure, you can leave out some. Essential are only function, module
and goal, but in Kephra we use all 6: function, module, feature, stage,
goal and sketch.
...
This prototype can then analyzed for memory and CPU usage and you get
clean numbers that are not muddied by other code that would be part of the
main program or a library. Also the real dependencies will be then clear.
This is invaluable information for proper planning and can be reused
for other projects.

A functional proto is a little sub-project with its own documentation,
tests and guidlines how to clean up the code, once its running.
The prototype is complete, when the cell has reached CP quality standards
and the enquired functionality is encapsuled as you intend it to use it later.
In short it's an ensurance, that the API you build on top can do what it
promises. And it also secures that you know how to implement base level
tasks your project relies upon. That alone avoids one source of unforeseen
project delays.

In case questions about the demo-ed technology show up later, you can
always fork and experiment with a functional prototype in a way that
avoids the complexeties of the main program.
...
The current stage will pick only a subset of the goal, but is obliged
to be consistent with the goal (checkable by software tests).
Because the goal is of course subject to changes - older stages might
...
which is a transfer from the current stage.

/dev itself will have 4 subfolder, for the cell types: /goal, /sketch,
/stage and function. They will have their own /doc, /lib, /bin ...
directories if needed. But the stages, which are storted under
/dev/stage/stagename have their modular and feature cells in subfolder,
because they got developed only for that stage.
Functional protos are under: /dev/function/topic/cell_name.

Every root directory of a cell has a status.cp.txt which reports the
