...
Because every kind of cell has a narrow goal, it becomes clear how much
...
just starting cell - its also gets adapted for the current task and thus
...
There are 6 classes of cells. Dependent on project size and type, you 
can leave out some. Essential are only function, module and goal,
but in Kephra we use all six. The types are sorted by level of abstraction.

=head3 Function

research
secure low level API you program against
...
dummy code with lot of comments and documentation
