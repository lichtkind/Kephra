...
but in Kephra we use all 6: proof, module, feature, stage, goal and sketch.
...
Every cell also needs a status.cp.txt, which describes if the work in
this cell has started, is in progress or completed. It also points out
what are the dependencies. Nice summaries can be automatically compiled
from these reports.

Same aplies for module and feature cells, bun only thy will surely
need a /lib subfolder since there are the main fils of the cell.
Other libs not developed here, should go in another subdirectory.

The root directories of each cell type like /dev/proof, but also
/dev/stage/name/module need to contain an index.cp.txt.
They list all contained cells and their status.
