...
#  {name=> 'type',       help=> 'type checker object',  code=> 'ref $value ~~ [Kephra::Base::Data::Type::class_names]', default=> $dummy_type, symbol => 'T' },
...
our @parametric_type_definitions =  (
    ...
    {name =>'typed_array', help=> 'array with typed elements',                                             
     code => '$param = Kephra::Base::Data::Type::Standard::set->get_type($param);for my $i(0..$#$value){my $error = $param->check_data($value->[$i]); return "array element $i $error" if $error}', 
    ...
    {name =>'typed_hash', help=> 'hash with typed values',                       parent=> 'hash_ref',  default=> {''=>1}, parameter => 'type_name',
     code => '$param = Kephra::Base::Data::Type::Standard::set->get_type($param);for my $k(keys %$value){my $error = $param->check_data($value->{$k}); return "hash value of key $k $error" if $error}' },
...
my $set = Kephra::Base::Data::Type::Set->new(); 

sub set  {$set}                                               #    -->  .tset
sub init {                                                    #    -->  .tset
    return $set unless $set->is_open();
