...
my $ts = Kephra::Base::Class::Instance::Type->new();
my $even = 'even';
...
is( ref $ts,                                   'Kephra::Base::Class::Instance::Type', 'created class type object');
is( scalar(my @a = $ts->list_names()) ,    $l, 'no types there yet');
...
is( $ts->get_default_value('int'),          0, 'knows default value of default types');

cmp_ok( $ts->add($even, $evendef),    '==', 1, 'type created');
is( scalar(@a = $ts->list_names()),      $l+1, 'now there is one more type');
ok( $even ~~ [$ts->list_names()],            , 'our type was registered');
cmp_ok( $ts->is_known($even),         '==', 1, 'now our special type is there');
cmp_ok( $ts->get_default_value($even),    '==', 2, 'set default value correctly');
like( $ts->check($even,'a',{}),    qr/number/, 'check rejected value: not number');
like( $ts->check($even,1.1,{}),   qr/integer/, 'check rejected value: not number');
like( $ts->check($even,1,{}),     qr/is even/, 'check rejected value: not even number');
...
is( scalar(@a = $ts->list_names()) ,       $l, 'no types there again');
...
cmp_ok( $ts->get_default_value($seven),'==',2, 'imported default value from parent correctly');
is( scalar(@a = $ts->list_names()) ,     $l+2, 'two types registered');
ok( $seven ~~ [$ts->list_names()],           , 'new type type registered');

my $cb = $ts->get_callback($even);
