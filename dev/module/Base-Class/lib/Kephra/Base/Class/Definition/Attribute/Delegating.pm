...
sub new {        # ~pkg %attr_def            --> ._ | ~errormsg
    ...
    my $error_start = ("delegating attribute $attr_def->{name}");
    ...
    return "constructor arguments of $error_start have to be in an array or (preferably) an hash"
        if exists $attr_def->{'init'} and ref $attr_def->{'init'} ne 'ARRAY' and ref $attr_def->{'init'} ne 'HASH';
    return "build code for the constructor arguments of $error_start have to be in an array or (preferably) an hash"
        if exists $attr_def->{'build'} and ref $attr_def->{'build'} ne 'ARRAY' and ref $attr_def->{'build'} ne 'HASH';
